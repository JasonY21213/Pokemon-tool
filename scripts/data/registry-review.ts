...
export async function buildRegistryReviewArtifacts(): Promise<RegistryReviewArtifacts> {
  ...
  const summary = { schemaVersion: 1, batchId: REGISTRY_REVIEW_BATCH, proposals: reviewed.length, perKind, registryEntriesBefore: baselineEntities.length, registryEntriesAfter: registryAfter.entities.length, accepted: accepted.length, manualReview: manualReview.length, rejected: rejected.length, blockingIdentityConflicts: rejected.length, pendingManualIdentityReviews: manualReview.length, existingFixtureIdsStable: baselineEntities.every((entity, index) => serializeJson(entity) === serializeJson(registryAfter.entities[index])), duplicateProjectIds: 0, duplicateExternalIds: 0, duplicateImmutableAnchors: 0 }

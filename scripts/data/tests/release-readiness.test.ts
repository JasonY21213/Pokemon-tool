...
import { loadPokemonRuntimeDataFrom, RuntimeDataLoadError } from '../../../src/lib/runtime-data/loader.ts'
...
test('loader validates the manifest before loading every base-path-aware runtime file', async () => {
  const responses = await runtimeResponses()
  const requested: string[] = []
  const data = await loadPokemonRuntimeDataFrom('/Pokemon-tool/', mockFetch(responses, requested))
...
test('loader returns stable user-facing failures for missing, malformed, incompatible, and unreachable data', async () => {
  const responses = await runtimeResponses()
  ...
  for (const entry of cases) {
    await assert.rejects(loadPokemonRuntimeDataFrom('/', mockFetch(responses, [], entry.overrides)), error => error instanceof RuntimeDataLoadError && error.code === entry.code && entry.message.test(error.message))
  }
  const unreachable = (async () => { throw new TypeError('offline') }) as typeof fetch
  await assert.rejects(loadPokemonRuntimeDataFrom('/', unreachable), error => error instanceof RuntimeDataLoadError && error.code === 'NETWORK' && /静态托管/.test(error.message))
})

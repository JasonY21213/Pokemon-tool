...
import { addTeamMember, defensiveSummary, offensiveCoverage, updateMemberMoves } from '../../../src/lib/runtime-data/team-builder.ts'
...
test('Team Builder caps members at six and normalizes each member to four unique allowed Moves', async () => {
  ...
  let members = [] as Array<{ memberId: string; formId: string; moveIds: string[] }>
  for (let index = 0; index < 7; index += 1) members = addTeamMember(members, { memberId: `member-${index}`, formId: 'form:0006:base', moveIds: ['move:0053'] }, formIds)
...
test('simple offensive coverage is deterministic and only reflects selected Move types against single defender types', async () => {
  const data = await dataPromise
  const coverage = offensiveCoverage([{ memberId: 'member-1', formId: 'form:0006:base', moveIds: ['move:0053'] }, { memberId: 'member-2', formId: 'form:0423:base', moveIds: ['move:0001'] }], data.moves, data.types)

...
test('runtime public/data emission is deterministic byte-for-byte', async () => {
  ...
  try {
    ...
    const first = await Promise.all(['species.json', 'forms.json', 'abilities.json', 'manifest.json'].map(file => readFile(join(temporaryRoot, 'public', 'data', file))))
    await emitRuntimeData(artifacts, outputRoot)
    const second = await Promise.all(['species.json', 'forms.json', 'abilities.json', 'manifest.json'].map(file => readFile(join(temporaryRoot, 'public', 'data', file))))

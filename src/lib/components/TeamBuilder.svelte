<script lang="ts">
  ...
  function addCurrent(): void { if (!selectedForm) return; const updated = addTeamMember(members, { memberId: `team-member-${nextMemberNumber}`, formId: selectedForm.formId, abilityId: null, moveIds: [] }, new Set(formMap.keys())); if (updated.length > members.length) nextMemberNumber += 1; members = updated }
  ...
  function setAbility(member: TeamMember, form: RuntimeForm, abilityId: string | null): void { members = members.map(item => item.memberId === member.memberId ? updateMemberAbility(item, abilityId, form) : item) }

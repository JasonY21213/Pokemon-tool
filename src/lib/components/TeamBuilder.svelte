<script lang="ts">
  ...
  import type { LearnsetRuntimeData, PokemonRuntimeData, RuntimeAbility, RuntimeForm, RuntimeMove } from '../runtime-data/types'
  ...
  function addMove(member: TeamMember): void { const moveId = selectedMoveByMember[member.memberId]; if (!moveId || !learnsets) return; const allowed = new Set(resolveEffectiveLearnsetMoveIds(learnsets, member.formId)); setMembers(members.map(item => item.memberId === member.memberId ? updateMemberMoves(item, [...item.moveIds, moveId], allowed) : item)) }
...
<section class="team-builder">
  <h2>队伍构建 <small>Team Builder</small></h2><p>队伍会自动保存在此浏览器中；不进行推荐、合法性判断或战术评分。</p>
  {#if learnsetsLoading}<p class="status" role="status" aria-live="polite"><strong>正在加载可学招式</strong> <small>Loading Learnsets</small>；队伍成员、形态、特性和已保存招式仍可使用。</p>{:else if learnsetsError}<div class="status error" role="alert"><strong>可学招式加载失败</strong> <small>Failed to Load Learnsets</small><p>队伍结构与已保存招式已保留；加入新招式暂不可用。</p><button type="button" onclick={onRequestLearnsets}>重试 <small>Retry</small></button></div>{/if}

...
<section class="pokemon-query">
  ...
  {#if selectedSpecies && selectedForm}<section class="detail" aria-live="polite">
    <header class="detail-identity"><p class="dex">全国图鉴 #{selectedSpecies.nationalDexNumber.toString().padStart(4, '0')} · Species</p><h2>{selectedSpecies.zhName} <small>{selectedSpecies.canonicalName}</small></h2><p>当前 Form：<strong>{formName(selectedForm)}</strong> <small>{selectedForm.canonicalName}</small></p></header>
    ...
    <section><h3>种族值</h3><div class="stats" aria-label="种族值">{#each statIds as stat}<span>{stat.toUpperCase()} <b>{selectedForm.baseStats[stat]}</b></span>{/each}<span class="bst">BST <b>{baseStatTotal(selectedForm)}</b></span></div></section>

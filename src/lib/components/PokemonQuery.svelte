<script lang="ts">
  ...
  import { abilitySlotLabel, formatLabel, pokemonDetailStatLabel, pokemonTypePresentation, typeLabel } from '../presentation/labels'
  ...
  function selectResult(result: ReturnType<typeof searchPokemon>[number]): void { const resolved = resolveSearchResult(result); selectedSpecies = resolved.species; selectedForm = data.forms.find(form => form.formId === resolved.formId) ?? null; selectedAbilityId = null; suggestionsOpen = false }
...
<section class="pokemon-query">
  <div class="pokemon-search-shell">
    <label class="search"><span>搜索宝可梦或形态</span><input role="combobox" aria-autocomplete="list" aria-controls="pokemon-search-results" aria-expanded={suggestionsOpen && Boolean(normalized) && results.length > 0} bind:value={query} oninput={() => suggestionsOpen = true} onkeydown={(event) => { if (event.key === 'Escape') suggestionsOpen = false }} placeholder="例如：妙蛙种子、Charizard、Mega X、6" /></label>
    {#if suggestionsOpen && normalized && results.length}<section id="pokemon-search-results" class="results" aria-label="搜索结果">{#each results as result (`${result.kind}:${result.form?.formId ?? result.species.speciesId}`)}<button class:active={result.form ? selectedForm?.formId === result.form.formId : selectedSpecies?.speciesId === result.species.speciesId} onclick={() => selectResult(result)}><span>#{result.species.nationalDexNumber.toString().padStart(4, '0')} · {result.kind === 'form' ? '形态' : '宝可梦'}</span><strong>{result.form ? formName(result.form) : result.species.zhName}</strong> <small>{result.form?.canonicalName ?? result.species.canonicalName}</small>{#if result.form}<em>{result.species.zhName}</em>{/if}</button>{/each}</section>

<script lang="ts">
  ...
  import { abilitySlotLabel, pokemonDetailStatLabel, pokemonTypePresentation } from '../presentation/labels'
  ...
  export let onAddToTeam: (selection: { form: RuntimeForm; abilityId: string | null; moveIds: string[] }) => void = () => {}
  let query = ''
  let selectedAbilityId: string | null = null
  let learnsetOpen = false
  let suggestionsOpen = false
  let temporaryMoveIds: string[] = []
  let teamAddStatus = ''
  ...
  function selectForm(form: RuntimeForm): void { selectedSpecies = data.species.find(species => species.speciesId === form.speciesId) ?? null; selectedForm = form; selectedAbilityId = null; temporaryMoveIds = []; teamAddStatus = '' }
  function selectResult(result: ReturnType<typeof searchPokemon>[number]): void { const resolved = resolveSearchResult(result); selectedSpecies = resolved.species; selectedForm = data.forms.find(form => form.formId === resolved.formId) ?? null; selectedAbilityId = null; temporaryMoveIds = []; teamAddStatus = ''; suggestionsOpen = false }
  function addCurrentToTeam(): void { if (!selectedForm || teamSize >= 6) return; onAddToTeam({ form: selectedForm, abilityId: selectedAbilityId, moveIds: temporaryMoveIds }); teamAddStatus = `已将${formName(selectedForm)}加入队伍。${learnsets ? '' : '招式暂未验证。'}` }
  function handleSearchKeydown(event: KeyboardEvent): void {
    if (event.key === 'Escape') suggestionsOpen = false
    else if (event.key === 'Enter' && suggestionsOpen && normalized && results.length) { event.preventDefault(); selectResult(results[0]) }
  }
...
<section class="pokemon-query">
  <div class="pokemon-search-shell">
    <label class="search"><span>搜索宝可梦或形态</span><input role="combobox" aria-autocomplete="list" aria-controls="pokemon-search-results" aria-expanded={suggestionsOpen && Boolean(normalized) && results.length > 0} bind:value={query} oninput={() => suggestionsOpen = true} onkeydown={handleSearchKeydown} placeholder="例如：妙蛙种子、Charizard、Mega X、6" /></label>
    {#if suggestionsOpen && normalized && results.length}<section id="pokemon-search-results" class="results" aria-label="搜索结果">{#each results as result, index (`${result.kind}:${result.form?.formId ?? result.species.speciesId}`)}<button class:default-result={index === 0} class:active={result.form ? selectedForm?.formId === result.form.formId : selectedSpecies?.speciesId === result.species.speciesId} onclick={() => selectResult(result)}><span>#{result.species.nationalDexNumber.toString().padStart(4, '0')} · {result.kind === 'form' ? '形态' : '宝可梦'}</span><strong>{result.form ? formName(result.form) : result.species.zhName}</strong> <small>{result.form?.canonicalName ?? result.species.canonicalName}</small>{#if result.form}<em>{result.species.zhName}</em>{/if}</button>{/each}</section>
    {:else if suggestionsOpen && normalized}<p class="search-empty" role="status">没有匹配结果。</p>{/if}
  </div>
  {#if selectedSpecies && selectedForm}<section class="detail" aria-live="polite">
    ...
    <section><h3>属性与分类</h3><div class="type-badges" aria-label="属性">{#each selectedForm.types as typeId (typeId)}{@const presentation = pokemonTypePresentation(typeId, data.types.find(type => type.typeId === typeId)?.canonicalName)}<span class="type-badge" style={`--type-background: ${presentation.background}; --type-foreground: ${presentation.foreground}`}>{presentation.label}</span>{/each}</div>{#if tagIds.length}<div class="tag-list" aria-label="已有分类标签">{#each tagIds as tagId}<span>{tagLabels[tagId] ?? tagId}</span>{/each}</div>{:else}<p>当前没有已接受的分类标签。</p>{/if}{#if selectedSpecies.growthRate.status === 'unresolved'}<p class="status">成长率资料在当前运行时数据中仍为未解决状态。</p>{/if}</section>
    <section><h3>种族值</h3><div class="stats" aria-label="种族值">{#each statIds as stat}<span>{pokemonDetailStatLabel(stat)} <b>{selectedForm.baseStats[stat]}</b></span>{/each}<span class="bst">种族值总和 <b>{baseStatTotal(selectedForm)}</b></span></div></section>
    <section><h3>特性</h3><ul>{#each abilities as { slot, ability } (ability.abilityId)}<li><strong>{abilitySlotLabel(slot)}</strong>：{ability.zhName ?? '未本地化'} <small>{ability.canonicalName}</small> · <em>{ability.mechanics.status === 'supported' ? '当前计算器支持' : '当前计算器未建模'}</em>{#if ability.zhDescription}<br />{ability.zhDescription}{/if}</li>{/each}</ul><label>用于防御相性预览的特性<select bind:value={selectedAbilityId}><option value={null}>不启用特性效果</option>{#each abilities as { slot, ability } (ability.abilityId)}<option value={ability.abilityId}>{abilitySlotLabel(slot)} · {ability.zhName ?? ability.canonicalName}（{ability.mechanics.status === 'supported' ? '支持' : '未建模'}）</option>{/each}</select></label>{#if selectedAbility?.mechanics.status === 'unsupported'}<p class="status">已选择的特性仍可能在游戏中有效；这里只表示当前计算器未建模。</p>{/if}</section>
    <section><h3>防御属性相性</h3><div class="matchup-groups">{#each matchup as group (group.multiplier)}<div class="matchup-row"><strong>{group.multiplier}×</strong><div class="type-badges">{#each group.entries as entry (entry.attackingTypeId)}{@const presentation = pokemonTypePresentation(entry.attackingTypeId, data.types.find(type => type.typeId === entry.attackingTypeId)?.canonicalName)}<span class="type-badge" style={`--type-background: ${presentation.background}; --type-foreground: ${presentation.foreground}`}>{presentation.label}</span>{/each}</div></div>{/each}</div>{#if adjustedMatchup.length}<p>特性预览变化：</p><ul class="adjusted-matchup">{#each adjustedMatchup as entry (entry.attackingTypeId)}{@const presentation = pokemonTypePresentation(entry.attackingTypeId, data.types.find(type => type.typeId === entry.attackingTypeId)?.canonicalName)}<li><span class="type-badge" style={`--type-background: ${presentation.background}; --type-foreground: ${presentation.foreground}`}>{presentation.label}</span><span>原始 {entry.rawMultiplier}× → 特性后 {entry.adjustedMultiplier}×</span></li>{/each}</ul>{/if}</section>
    ...
    <section class="learnset-entry"><h3>可学招式 <small>Learnsets</small></h3>{#if effectiveMoveCount === null}<p>打开可学招式后加载。<small>Load on demand.</small></p>{:else}<p>当前 Form 有 {effectiveMoveCount} 个固定来源关联招式；跨世代关联不代表当前游戏合法性。</p>{/if}{#if !learnsetOpen}<button type="button" onclick={() => learnsetOpen = true}>打开可学招式 <small>Open Learnsets</small></button>{/if}<div class="query-team-add">{#if !learnsets}<p class="status">{learnsetsError ? '可学招式加载失败；已有招式会保留并暂未验证。' : '可学招式尚未加载；已有招式会保留并暂未验证。'}</p>{/if}<button type="button" disabled={teamSize >= 6} onclick={addCurrentToTeam}>{teamSize >= 6 ? '队伍已满' : `加入队伍（当前临时招式 ${temporaryMoveIds.length} / 4）`}</button>{#if teamAddStatus}<p class="status" aria-live="polite">{teamAddStatus}</p>{/if}</div></section>
    {#if learnsetOpen}<LearnsetExplorer {data} {selectedForm} {learnsets} {learnsetsLoading} {learnsetsError} {onRequestLearnsets} bind:movesetIds={temporaryMoveIds} />{/if}
  </section>{/if}
</section>

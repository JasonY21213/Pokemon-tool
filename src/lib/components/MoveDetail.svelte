<script lang="ts">
  ...
  $: typeName = data.types.find(type => type.typeId === move.typeId)?.canonicalName ?? move.typeId
</script>
<div class="move-detail" aria-live="polite"><h3>{move.zhName ?? '未本地化'} <small>{move.canonicalName}</small></h3><p>{move.zhDescription ?? '暂无中文简介。'}</p><div class="move-mechanics"><span>属性 <b>{typeName}</b></span><span>分类 <b>{category(move.category)}</b></span><span>威力 <b>{numeric(move.power)}</b></span><span>命中 <b>{accuracy(move.accuracy)}</b></span><span>PP <b>{numeric(move.pp)}</b></span><span>优先度 <b>{move.priority}</b></span></div>{#if move.damageSupport.status === 'supported'}<p>核心伤害计算器：支持普通单目标公式。</p>{:else if move.damageSupport.status === 'non-damaging'}<p>核心伤害计算器：变化招式，不使用普通伤害公式。</p>{:else if move.damageSupport.status === 'unsupported'}<p>核心伤害计算器：暂不支持，{support[move.damageSupport.reason]}</p>{:else}<p>核心伤害计算器：资料不足，无法可靠计算。</p>{/if}</div>

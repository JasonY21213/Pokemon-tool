import type { PokemonRuntimeData, RuntimeAbility, RuntimeForm, RuntimeGrowthRate, RuntimeManifest, RuntimeNature, RuntimeSpecies, RuntimeType } from './types'
...
export async function loadPokemonRuntimeData(): Promise<PokemonRuntimeData> {
  const [species, forms, abilities, types, natures, growthRates, manifest] = await Promise.all([
  ...
  return { species, forms, abilities, types, natures, growthRates, manifest }
}
